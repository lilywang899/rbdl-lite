...
RobotModel::RobotModel(std::string& path) {
    model_ = new Model();
    rbdl_check_api_version(RBDL_API_VERSION);

    if (!Addons::URDFReadFromFile(path.c_str(), model_, false, true)) {
    ...
    std::cout << path << "Constructed successfully" << std::endl;
}
...
void RobotModel::UpdateSystem(const dynacore::Vector &q, const dynacore::Vector &qdot) {
    dynacore::Vector qddot = qdot;
    qddot.setZero();
