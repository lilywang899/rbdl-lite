...
struct RBDL_DLLAPI Body {
    Body() : mMass(0.),
             ...
             mIsVirtual(false){};
    Body(const Body &body) : mMass(body.mMass),
   ...
   */
    Body(const double &mass,
         ...
         const Math::Vector3d &gyration_radii) : mMass(mass),
                                                 mCenterOfMass(com),
                                                 mIsVirtual(false) {
   ...
   */
    Body(const double &mass,
         const Math::Vector3d &com,
         const Math::Matrix3d &inertia_C) : mMass(mass),
                                            mCenterOfMass(com),
                                            mInertia(inertia_C),
                                            mIsVirtual(false) {}
   ...
   */
    void Join(const Math::SpatialTransform &transform, const Body &other_body) {
        ...
        LOG << "other_com = " << std::endl
            << other_com.transpose() << std::endl;
        LOG << "rotation = " << std::endl
            << transform.E << std::endl;
        ...
        LOG << "inertia_other = " << std::endl
            << inertia_other << std::endl;
        ...
        LOG << "inertia_summed  = " << std::endl
            << inertia_summed << std::endl;
        ...
        LOG << "new_inertia  = " << std::endl
            << new_inertia << std::endl;
