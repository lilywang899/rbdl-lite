...
void CalcConstraintsPositionError(
    ...
    for (unsigned int i = 0; i < CS.mLoopConstraintIndices.size(); i++) {
        ...
        pos_p = CalcBodyToBaseCoordinates(model, Q, CS.body_p[lci], CS.X_p[lci].r, false);
        pos_s = CalcBodyToBaseCoordinates(model, Q, CS.body_s[lci], CS.X_s[lci].r, false);
...
void CalcConstraintsJacobian(
    ...
    for (unsigned int i = 0; i < CS.mLoopConstraintIndices.size(); i++) {
        ...
        if (prev_body_id_1 != CS.body_p[c]
            ...
            pos_p = CalcBodyToBaseCoordinates(model, Q, CS.body_p[c], CS.X_p[c].r, false);
            rot_p = CalcBodyWorldOrientation(model, Q, CS.body_p[c], false).transpose() * CS.X_p[c].E;
...
void CalcConstrainedSystemVariables(
    ...
    for (unsigned int i = 0; i < CS.mContactConstraintIndices.size(); i++) {
        ...
        if (prev_body_id != CS.body[c] || prev_body_point != CS.point[c]) {
            gamma_i = CalcPointAcceleration(model, Q, QDot, CS.QDDot_0, CS.body[c], CS.point[c], false);
    ...
    for (unsigned int i = 0; i < CS.mLoopConstraintIndices.size(); i++) {
        ...
        SpatialVector acc_p = CalcPointAcceleration6D(model, Q, QDot, VectorNd::Zero(model.dof_count), CS.body_p[c], CS.X_p[c].r, false);
        SpatialVector acc_s = CalcPointAcceleration6D(model, Q, QDot, VectorNd::Zero(model.dof_count), CS.body_s[c], CS.X_s[c].r, false);
            ...
            if (lambda != 0) {
                CS.d_pA[lambda] = CS.d_pA[lambda]
                    + model.X_lambda[i].applyTranspose(
                        CS.d_pA[i] + (model.multdof3_U[i] * model.multdof3_Dinv[i] * CS.d_multdof3_u[i]));
            }
        } else if (model.mJoints[i].mDoFCount == 1
                   && model.mJoints[i].mJointType != JointTypeCustom) {
...
void ForwardDynamicsContactsKokkevis(
    ...
    for (ci = 0; ci < CS.size(); ci++) {
        ...
        if (CS.constraintType[ci] == ConstraintSet::ContactConstraint) {
            ...
            {
                SUPPRESS_LOGGING;
                CS.point_accel_0[ci] = CalcPointAcceleration(model, Q, QDot, CS.QDDot_0, CS.body[ci], CS.point[ci], false);
    ...
    for (ci = 0; ci < CS.size(); ci++) {
        ...
        case ConstraintSet::ContactConstraint:
            ...
            point_global = CalcBodyToBaseCoordinates(model, Q, CS.body[ci], CS.point[ci], false);

            LOG << "point_global = " << point_global.transpose() << std::endl;

            CS.f_t[ci] = SpatialTransform(Matrix3d::Identity(), -point_global)
                             .applyAdjoint(SpatialVector(0., 0., 0., -CS.normal[ci][0], -CS.normal[ci][1], -CS.normal[ci][2]));
