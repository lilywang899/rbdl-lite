    ...
    while (j != 0) {
        ...
        if (model.mJoints[j].mJointType != JointTypeCustom) {
            if (model.mJoints[j].mDoFCount == 1) {
                ...
                G.block(0, q_index, 6, 3) = ((point_trans
                                              * model.X_base[j].inverse())
                                                 .toMatrix()
                                             * model.multdof3_S[j])
                                                .block(0, 0, 6, 3);
            ...
            G.block(0, q_index, 6, model.mCustomJoints[k]->mDoFCount) = ((point_trans
                                                                          * model.X_base[j].inverse())
                                                                             .toMatrix()
                                                                         * model.mCustomJoints[k]->S)
                                                                            .block(
        ...
        if (model.mJoints[j].mJointType != JointTypeCustom) {
            if (model.mJoints[j].mDoFCount == 1) {
                ...
                G.block(0, q_index, 6, 3) =
                    (base_to_body * model.X_base[j].inverse()).toMatrix() * model.multdof3_S[j];
            }
        } else if (model.mJoints[j].mJointType == JointTypeCustom) {
            unsigned int k = model.mJoints[j].custom_joint_index;

            G.block(0, q_index, 6, model.mCustomJoints[k]->mDoFCount) =
                (base_to_body * model.X_base[j].inverse()).toMatrix() * model.mCustomJoints[k]->S;
        }
